...
void CustomGraphicsScene::DrawCircle(double x, double y, double Radius, bool usingFill){
    circle = new CustomCircleItem(x, y, Radius);  // 创建自定义圆形项
    qDebug() << "[DEBUG] The width of the pen " << LineColor.width();
...
void CustomGraphicsScene::DrawLine(double x1, double y1, double x2, double y2){
    line = new CustomLineItem(QLineF({x1, y1}, {x2, y2}));  // 创建自定义圆形项
    qDebug() << "[DEBUG] The width of the pen " << LineColor.width();
    line->setPen(LineColor);
    line->R=LineColor.color().red();
    line->G=LineColor.color().green();
    line->B=LineColor.color().blue();
    addItem(line);  // 添加圆形项到场景
}

void CustomGraphicsScene::DrawText(double x, double y, QString msg){
    text = new CustomTextItem(msg);
    text->setX(x);
    text->setY(y);
    addItem(text);  // 添加圆形项到场景
}

void CustomGraphicsScene::DrawRect(double x1, double y1, double x2, double y2, bool usingFill){
    rectangle = new CustomRectangleItem(QRectF(QPointF(x1, y1), QPointF(x2, y2)));  // 创建自定义圆形项
    qDebug() << "[DEBUG] The width of the pen " << LineColor.width();
    rectangle->setPen(LineColor);
    if (usingFill)
        rectangle->setBrush(QColor(FillColor.color().red(), FillColor.color().green(), FillColor.color().blue()));
    // rectangle->R=LineColor.color().red();
    // rectangle->G=LineColor.color().green();
    // rectangle->B=LineColor.color().blue();
    addItem(rectangle);  // 添加圆形项到场景
}
...
void CustomGraphicsScene::mousePressEvent(QGraphicsSceneMouseEvent *event) {
    ...
    {  // 新增：如果正在添加直线
        ...
        if (firstPoint.isNull()) {
            ...
            rectangle = new CustomRectangleItem(rect);
            if (usingFill)
                rectangle->setBrush(QColor(FillColor.color().red(), FillColor.color().green(), FillColor.color().blue()));
            rectangle->setPen(LineColor);
            addItem(rectangle);
