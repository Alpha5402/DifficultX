...
Operation extractVariables(const QString &input) {
    // 定义不同的正则表达式模式
    Operation ans;
    QRegularExpression circleRegex(R"((circle|fillcircle)\(([^)]*)\))");
    QRegularExpression lineAndRectRegex(R"((line|rectangle|fillrectangle)\(([^)]*)\))");
    ...
    QRegularExpression textStyleRegex(R"((_tcscpy_s\(f\.lfFaceName,\ _T\(\"([^\"]+)\"\)\);)");
    QRegularExpression textRegex(R"(outtextxy\(\d+,\s*\d+,\s*_T\(".*"\)\);)");
    ...
    QRegularExpressionMatch textStyleMatch = textStyleRegex.match(input);
    QRegularExpressionMatch textMatch = textRegex.match(input);
    ...
    }  else if (textStyleMatch.hasMatch()) {
        QString para_1 = textStyleMatch.captured(1); // 匹配到的操作名称

        ans.type = "settextstyle";
        ans.stringPara = para_1;
    }  else if (textMatch.hasMatch()) {
        QString para_1 = textMatch.captured(1); // 匹配到的操作名称
        QString para_2 = textMatch.captured(2); // 匹配到的操作名称
        QString para_3 = textMatch.captured(3); // 匹配到的操作名称

        ans.type = "outtextxy";
        ans.parameter_1 = para_1.toDouble();
        ans.parameter_2 = para_2.toDouble();
        ans.stringPara = para_3;
    } else {
...
void MainWindow::openImageEditor(){
    ...
    for (auto element = Handle.begin(); element != Handle.end(); ++element) {
        ...
        if ((*element).type == "circle") {
            ...
            imageditor->drawingArea->scene->DrawCircle((*element).parameter_1, (*element).parameter_2, (*element).parameter_3 * 2, false);
        } else if ((*element).type == "fillcircle") {
            qDebug() << "[INFO] Circle detected";
            imageditor->drawingArea->scene->DrawCircle((*element).parameter_1, (*element).parameter_2, (*element).parameter_3 * 2, true);
            ...
            qDebug() << "[INFO] Circle detected";
            imageditor->drawingArea->scene->DrawRect((*element).parameter_1, (*element).parameter_2, (*element).parameter_3, (*element).parameter_4, false);
        } else if ((*element).type == "fillrectangle") {
            qDebug() << "[INFO] Circle detected";
            imageditor->drawingArea->scene->DrawRect((*element).parameter_1, (*element).parameter_2, (*element).parameter_3, (*element).parameter_4, true);
        ...
        } else if ((*element).type == "settextstyle") {
            qDebug() << "[INFO] Bkcolor detected";
            QFont font((*element).stringPara);
            imageditor->fontLabel->setText(QString("设置文本字体: %1").arg((*element).stringPara));
            font.setPixelSize(imageditor->fontSizeComboBox->currentText().toInt());
            imageditor->fontLabel->setFont(font);
            imageditor->textFont = font;
            imageditor->drawingArea->scene->text->setFont(imageditor->textFont);
        } else if ((*element).type == "outtextxy") {
            qDebug() << "[INFO] Bkcolor detected";
            imageditor->drawingArea->scene->DrawText((*element).parameter_1, (*element).parameter_2, (*element).stringPara);
        }
    }
...
{
    setWindowTitle("DifficultX - 图形化编辑器");
    // 创建中央小部件
    ...
    QObject::connect(timer, &QTimer::timeout, this, &MainWindow::autoSaving);

    // 设置定时器的间隔时间（例如，1000 毫秒，即 1 秒）
    timer->start(60000);
}

void MainWindow::autoSaving(){
    QString code = textEdit->toPlainText();
    QString docsPath = QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation);
    QString tempPath = docsPath + "/DifficultX/Backup";
    ...
    if (!dir.exists()) {
        if (!dir.mkpath(tempPath)) {
            QMessageBox::critical(this, "Error", "Failed to create saving directory.");
            return;
        }
    }

    QString finalPath = tempPath + "/Backup_" + formattedDateTime + ".cpp";

    qDebug() << "[AutoSaving] Saving to " + finalPath;

    QString filePath = createTempFilePath(finalPath);
...
void MainWindow::compileAndRunCode() {
    ...
    QString cmd2 = "\"C:/Program Files/Microsoft Visual Studio/2022/Community/Common7/Tools/VsDevCmd.bat\" -arch=amd64 && cl temp_code.cpp /Fe:tempCode.exe /link /LIBPATH:\"C:/Program Files/Microsoft Visual Studio/2022/Community/VC/Auxiliary/VS/lib/x64\" /LIBPATH:\"C:/Program Files (x86)/Windows Kits/10/Lib/10.0.22621.0/um/x64\" /LIBPATH:\"C:/Program Files (x86)/Windows Kits/10/Lib/10.0.22621.0/ucrt/x64\" EasyXa.lib user32.lib gdi32.lib kernel32.lib shell32.lib /SUBSYSTEM:CONSOLE\r\n";
    ...
    if (!process.waitForFinished(-1)) {
     ...
     if (errorString.contains("error", Qt::CaseInsensitive)) {
         qDebug() << "Error found, terminating process.";
         int i = 0, j = 0, cnt = 0;
         for(i = 0; i < errorString.size(); i++){
             if (errorString.mid(i, 70) == "**********************************************************************"){
                 cnt++;
                 if (cnt >= 2) break;
             }
         }
         ...
         for(j = errorString.size() - 1; j >= 0; j--){
             if (errorString.mid(j, 3) == "C:\\"){
                 break;
             }
         }
         QString errorText = errorString.mid(i, j - i);
         QMessageBox::critical(this, "编译错误", errorText);
         return;
     }
